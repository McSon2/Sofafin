...
final class PlaybackEngine: NSObject, @MainActor AVPlayerViewControllerDelegate {
    ...
    private func load(_ requested: MediaItem, startTime: Double?, previousDidFinish: Bool) async {
        ...
        // Confier au lecteur l'adresse de la première plutôt que celle du
        // catalogue le prive d'un choix qu'il fait mal.
        let url = await PlaybackManifest.resolved(plan.url)
        logPlayerURL(url)
